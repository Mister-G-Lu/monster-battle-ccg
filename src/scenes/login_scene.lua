...
function meta:showFrameByPlatform()
    ...
    if device.platform == "ios" then
        ...
        if lang_now == login_logic.locale_list[cc.LANGUAGE_CHINESE] then    --ios国内
...
function meta:setLoginBtnState(status_code, arg2)
    ...
    elseif status_code == 2 then    --不可点击
        if self.fbBtn then
