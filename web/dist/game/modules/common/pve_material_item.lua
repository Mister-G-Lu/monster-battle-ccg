...
local meta = class("pve_material_item",function (node)
    if node then
        return node
    end
    return ui_helper:LoadCocosUI("interface/battle/battle_end2/reward_item_template.csb")
end)
...
function meta:ShowReward(reward_info)
    ...
    self:AddClick(
        function (pos)
            graphic:DispatchEvent("show_pve_reward_tips", reward_info, pos)
        end,
        function ()
            graphic:DispatchEvent("hide_pve_reward_tips")
        end
    )
end
...
function meta:ShowItem(item_info)
    ...
    self.icon_img:loadTexture(resource:GetItemIcon(config.res_path))
    if item_info.item_num == 0 then
        ui_helper:SetText(self.num_txt, text_loader:GetText("battle_result_tips_info"))
    else
