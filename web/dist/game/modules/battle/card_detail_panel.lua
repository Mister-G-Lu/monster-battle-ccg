...
function meta:ShowCard(info,idx)
    if info  then
        ...
        -- --demo_node:setScale(1)
        if idx == 1 then
               self.card:setPosition(cc.p(472,344))
        elseif idx == 2 then
            self.card:setPosition(cc.p(390,364))

        elseif idx == 3 then
            self.card:setPosition(cc.p(250,364))

        else
            self.card:setPosition(cc.p(168,344))
...
function meta:RegisterEvent()
    battle_logic:RegisterEvent("show_hand_card_detail", function (info,idx)
        self.is_show = true
        self.time_count = DELAY_TIME
        self.show_info = info
        self:setVisible(true)
    ...
    battle_logic:RegisterEvent("hide_hand_card_detail", function ()
        self:Hide()
    end)
end
