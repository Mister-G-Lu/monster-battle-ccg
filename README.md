...
A fully offline, single-player version of the English Card Battle game — originally an online-only Cocos2d-x Android title. This build runs entirely on-device with no Internet connection required, and the home bar's **Adventure** button opens **The Shadow Road** — a handcrafted 4-act campaign fought on the native battle engine. It is the only fight in the build: the hidden Battle compatibility door routes to the same campaign, and the stock PvE mode (Gerbip Tide) is [archived](archive/gerbip_tide/README.md).
...
luajit tests/campaign_client_test.lua # client queue + result-close return to campaign
luajit tests/campaign_ui_test.lua     # native Adventure map UI: rows, refresh, back routing
...
luajit tests/home_battle_button_test.lua  # home Adventure button -> campaign world system
...
and persistence across re-login. The campaign section in
`tests/integration_test.lua` additionally verifies the post-battle contract
consumed by Adventure: `cmd_battle_over` carries a map-refresh node id, a
non-nil MVP table, a displayable EXP reward, the next unlocked node, and the
pending recruit draft.
...
`tests/campaign_ui_test.lua` builds the real native `campaign_panel` against a
Cocos node model and asserts that all 19 encounter rows render in the scroll
container, a post-battle refresh advances the highlighted node without losing
the map, Back returns through the world-system router, and a failed progress
read still leaves a usable canonical road on screen.

`tests/home_battle_button_test.lua` builds the real `home_panel` against a stub
`.csb` and asserts the button labelled **Adventure** (`pvebtn`) is the campaign
door, that the hidden Battle compatibility door routes to the same place, and
that neither path can fall back to the archived stock PvE list.
...
│   │   ├── world/system/home_panel.lua  # Home Adventure button -> campaign world system
│   │   ├── world/system/pve_panel.lua   # Legacy PvE list (Gerbip Tide archived)
...
│   ├── campaign_client_test.lua  # Client queue + post-result campaign return
│   ├── campaign_ui_test.lua      # Native Adventure map UI / refresh / back routing
...
│   ├── home_battle_button_test.lua  # Home Adventure button -> campaign world system
